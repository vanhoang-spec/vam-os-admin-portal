-- Migration 032: Season 11 Closed-Month Dashboard Fix
-- Resolves the "open-month bleed" issue where dashboard selects current open month (April)
-- instead of the officially closed month (March).
-- Relies on v_season_latest_closed_month established in migration 031.
-- Refactors get_operations_dashboard_data to use explicit closed month snapshots for KPIs.
