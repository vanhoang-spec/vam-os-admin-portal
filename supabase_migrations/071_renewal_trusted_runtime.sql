-- =============================================================================
-- MIGRATION 071
-- Season 12 Returning-Mentor Renewal — trusted runtime RPC foundation
--
-- Creates SEVEN functions in the public schema and NOTHING else. No table, no
-- column, no index, no policy, no grant on any pre-existing table, no seed row,
-- no audit vocabulary change. M070's public.person_season_invites is read and
-- written by these functions and is not re-authored by this migration in any
-- way.
--
-- WHY THESE FUNCTIONS EXIST AT ALL
--   Four of the five renewal operations perform two or more writes that must
--   either all land or none land, and in three of them a gate-then-write
--   sequence in application code is a TOCTOU race with a canonical outcome:
--
--     accept   applications INSERT + invite claim. Two concurrent submits of
--              one invite must produce exactly one application. A read-decide-
--              write in Node cannot do this; a locked transaction can.
--     decline  invite claim + membership opt-out, refused outright when an
--              accepted renewal already exists. The refusal must hold against
--              a concurrent accept, which means it must be evaluated under the
--              same lock as the write it guards.
--     create   the accepted-renewal refusal, the one-live-invite refusal, the
--              token_hash INSERT and the create_renewal_invite audit row are
--              one decision. Split across statements, a partial failure leaves
--              a live bearer token with no audit record of who minted it.
--     revoke   revoked_at plus its audit row, against the exact intended live
--              invite, under a row lock so a concurrent submit cannot slip
--              between the read and the write.
--     confirm  the canonical mentor_profiles write, its drift refusal and the
--              confirm_renewal audit row that records what the admin actually
--              authorised. The audit is written in the same transaction as the
--              mutation it attests, so an unaudited canonical write is not
--              representable.
--
-- WHAT IS DELIBERATELY NOT HERE
--   * No membership logic is moved into approveApplication, and
--     approveApplication is not modified. The confirm orchestration calls it.
--   * No new membership status, no new lifecycle vocabulary, no new audit
--     action_type. M070 already added the only three this runtime writes.
--   * No mentee renewal. Every entry point refuses a role other than 'mentor';
--     see Section 2.
...
--   p_token_hash and it is pinned to ^[0-9a-f]{64}$ before it is used. The raw
--   43-character token is hashed in the Node process by
--   lib/renewal-invite-token.ts and never crosses the SQL boundary, so it
--   cannot appear in pg_stat_statements, in a log_min_duration_statement line,
--   or in a PostgREST request body that some proxy retains. The digest is not
--   a credential outside this database: possessing it lets nobody call these
--   functions, because every one of them is service_role-only.
-- =============================================================================
