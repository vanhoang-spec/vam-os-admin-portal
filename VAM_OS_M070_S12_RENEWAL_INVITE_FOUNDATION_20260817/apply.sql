-- =============================================================================
-- VAM OS — M070 PRODUCTION APPLY
-- Season 12 Returning-Mentor Renewal — person-bound invite foundation
--
-- Identical in effect to supabase_migrations/070_person_season_invites.sql;
-- this copy adds the Section 0 re-assertion block below. Everything from
-- `-- ── 1. Prerequisites and unapplied proof` onward is BYTE-IDENTICAL to the
-- canonical migration, and __tests__/m070-renewal-invite-foundation.test.ts
-- fails the build if the two ever diverge.
--
-- THIS FILE DOES NOT DEPEND ON preflight.sql HAVING BEEN RUN.
-- Section 0 re-asserts, INSIDE this transaction and before the first mutation,
-- every material condition the standalone preflight proves: environment
-- identity, the exact type of every foreign-key target column, the shape of
-- the shared updated_at function, the complete absence of M070, and that the
-- live audit vocabulary is one of the two baselines this Production database
-- can legitimately be in. Running the preflight is still recommended — it
-- reports the same refusals read-only, without taking a lock — but skipping
-- it, or a baseline that drifted after it passed, cannot make this apply
-- unsafe: the transaction aborts before it writes anything.
--
-- DO NOT RUN THIS FILE UNTIL INDEPENDENT SECURITY REVIEW HAS PASSED.
-- =============================================================================
...
declare
  ...
  v_raw_n      integer;
  v_quote_n    integer;
  -- The FK target contract. Every one of these columns is referenced by a
  -- column this migration declares as uuid; a type mismatch is 42804 at DDL
  -- time, which must abort here rather than half-way through Section 2.
  v_fk_targets constant text[] := array[
    'people.id', 'programs.id', 'seasons.id', 'admin_users.id', 'applications.id'
  ];
  -- BYTE-IDENTICAL to the array in the canonical migration Section 6 and in
  -- preflight.sql, and proven so by test. The canonical representation lives in
...
begin
  ...
  if exists (select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    ...
    raise exception 'M070 ABORTED [ENV_NOT_PRODUCTION]: vam062_* functions exist — this is Staging.';
  end if;

  -- ── B. gen_random_uuid() must resolve ────────────────────────────────────
  -- The primary key default calls it. Every existing table in this database
  -- already does, so this is a shape check, not a version guess.
  if to_regprocedure('gen_random_uuid()') is null
     and to_regprocedure('public.gen_random_uuid()') is null then
    raise exception 'M070 ABORTED [UUID_FN_MISSING]: gen_random_uuid() does not resolve; the primary key default cannot be created.';
  end if;

  -- ── C. Foreign-key target contract ───────────────────────────────────────
  ...
  if v_txt is not null then
    raise exception 'M070 ABORTED [FK_TARGET_TYPE]: %. Every foreign key this migration declares is uuid.', v_txt;
  end if;

  -- Each FK target column must carry a primary or unique key, or REFERENCES
  -- fails with 42830.
  ...
  if v_txt is not null then
    raise exception 'M070 ABORTED [FK_TARGET_KEY]: % carr(y|ies) no single-column primary or unique key; REFERENCES would fail with 42830.', v_txt;
  end if;

  -- ── D. The shared updated_at function, by shape not by name ──────────────
  if not exists (
    ...
    raise exception 'M070 ABORTED [UPDATED_AT_FN_SHAPE]: public.set_updated_at() is absent or is not a zero-argument plpgsql trigger function. This migration reuses it and creates no trigger infrastructure of its own.';
  end if;

  -- ── E. M070 is completely unapplied ──────────────────────────────────────
  ...
  -- ── F. The audit vocabulary baseline ─────────────────────────────────────
  -- Section 6 builds its replacement from the LIVE set, so it cannot delete a
  -- Production value. This block additionally proves the live set is one of the
  -- two baselines this database can legitimately be in, so an unexplained
  -- vocabulary stops the run instead of being carried forward silently.
  ...
  if exists (select 1 from pg_class
    ...
    raise exception 'M070 ABORTED [AUDIT_INSERT_BLOCKED]: FORCE ROW LEVEL SECURITY is set on admin_audit_log, so the SECURITY DEFINER audit INSERT the renewal runtime performs is filtered by policy even running as the table owner. RLS being merely ENABLED is the accepted baseline and is fine; FORCE is not.';
  end if;
  ...
  if v_n <> 1 then
    raise exception 'M070 ABORTED [AUDIT_VOCAB_SHAPE]: % CHECK constraint(s) govern admin_audit_log.action_type, expected exactly 1.', v_n;
  end if;
  ...
  if v_txt is distinct from v_conname then
    raise exception 'M070 ABORTED [AUDIT_VOCAB_SHAPE]: the action_type CHECK is named %, expected %.', coalesce(v_txt, '<null>'), v_conname;
  end if;
  if v_validated is not true then
    raise exception 'M070 ABORTED [AUDIT_VOCAB_NOT_VALIDATED]: % is NOT VALID.', v_conname;
  end if;
  ...
  if v_quote_n <> 2 * v_raw_n
     or v_raw_n <> coalesce(array_length(v_actual, 1), 0) then
    raise exception 'M070 ABORTED [AUDIT_VOCAB_SHAPE]: % is not a parseable, duplicate-free list: %.', v_conname, v_def;
  end if;

  select array_agg(x order by x) into v_unexpected from unnest(v_m070) x where x = any (v_actual);
  if v_unexpected is not null then
    raise exception 'M070 ABORTED [AUDIT_VOCAB_M070_PRESENT]: the vocabulary already admits %.', array_to_string(v_unexpected, ', ');
  end if;
  ...
  if v_missing is not null or v_unexpected is not null then
    raise exception 'M070 ABORTED [AUDIT_VOCAB_UNEXPECTED]: % admits % value(s), which is neither BASE52 (the canonical 52) nor BASE53 (those 52 + %). MISSING: %. UNEXPECTED: %.',
  ...
  raise notice 'M070 Section 0 passed: environment, gen_random_uuid, the five foreign-key targets, public.set_updated_at() shape, complete absence of M070, and the % audit vocabulary baseline all re-asserted inside this transaction.',
    case when v_m069 = any (v_actual) then 'BASE53' else 'BASE52' end;
end
$m070_guard$;
