...
describe("preview and SQL package safety",()=>{
  ...
  it("migration 062 apply guard uses explicit pretty-form output and keeps the legacy vocabulary strict",()=>{const sql=read("supabase_migrations/062_review_only_account_admin_rls_foundation.sql");const literal="CHECK (action_type = ANY (ARRAY[''create_admin_user''::text, ''update_admin_user''::text, ''reactivate_admin_user''::text, ''deactivate_admin_user''::text, ''remove_admin_access''::text, ''sync_auth''::text, ''unknown''::text])) NOT VALID";expect(sql).toContain("pg_get_constraintdef(c.oid, true)");expect(sql).toContain("c.convalidated=false");expect(sql).toContain(`pg_get_constraintdef(c.oid, true)='${literal}'`);const extractedValues=Array.from(literal.matchAll(/''([^']+)''::text/g), (m)=>m[1]);expect(extractedValues).toEqual(["create_admin_user","update_admin_user","reactivate_admin_user","deactivate_admin_user","remove_admin_access","sync_auth","unknown"]);expect(sql).toContain("c.conname='admin_audit_log_action_type_check'");expect(sql).toContain("c.contype='c'");});
  it("rollback guard uses the same explicit pretty-form comparison and stays strict",()=>{const sql=read("docs/audits/sql/design_only/VAM_OS_ACCOUNT_ADMIN_RLS_ROLLBACK.sql");const literal="CHECK (action_type = ANY (ARRAY[''create_admin_user''::text, ''update_admin_user''::text, ''reactivate_admin_user''::text, ''deactivate_admin_user''::text, ''remove_admin_access''::text, ''sync_auth''::text, ''unknown''::text, ''import_participant_membership''::text, ''link_person_auth''::text, ''reconcile_person_auth''::text, ''create_membership''::text, ''add_membership_role''::text, ''remove_membership_role''::text, ''pause_membership''::text, ''withdraw_membership''::text, ''opt_out_membership''::text, ''cancel_membership''::text, ''reactivate_membership''::text])) NOT VALID";expect(sql).toContain("pg_get_constraintdef(c.oid, true)");expect(sql).toContain("c.convalidated=false");expect(sql).toContain(`pg_get_constraintdef(c.oid, true)='${literal}'`);});
  ...
  // The post-apply verifier now compares quals after whitespace-only
  // normalisation (C1, 2026-08-03) because pg_get_expr renders sub-SELECTs
  // across multiple lines. The bans below are what keep that narrow: a
  // normalisation class that also swallowed parentheses ("[[:space:]()]") or
  // punctuation would destroy grouping, which is exactly the failure these
  // assertions exist to prevent.
  const POLICY_QUAL_COMPARISON=/qual=|btrim\(regexp_replace\(p\.qual,'\[\[:space:\]\]\+',' ','g'\)\)=btrim\(regexp_replace\(e\.q,'\[\[:space:\]\]\+',' ','g'\)\)/;
  it("preserves policy grouping through exact expression comparison",()=>{for(const path of ["supabase_migrations/062_review_only_account_admin_rls_foundation.sql","docs/audits/sql/design_only/VAM_OS_ACCOUNT_ADMIN_RLS_PREFLIGHT.sql","docs/audits/sql/design_only/VAM_OS_ACCOUNT_ADMIN_RLS_POST_APPLY_VERIFY.sql"]){const sql=read(path);expect(sql).not.toContain("[[:space:]()]");expect(sql).not.toContain("punctuation");expect(sql).toMatch(POLICY_QUAL_COMPARISON)}});
  it("uses independent exact post-apply policy specifications",()=>{const sql=read("docs/audits/sql/design_only/VAM_OS_ACCOUNT_ADMIN_RLS_POST_APPLY_VERIFY.sql");expect(sql).toMatch(POLICY_QUAL_COMPARISON);expect(sql).not.toContain("p.qual like");expect(sql).not.toContain("p.qual ilike");expect(sql).toContain("s.season_id IS NOT NULL");expect(sql).toContain("policy_inventory");expect(sql).toContain("package_grants")});
