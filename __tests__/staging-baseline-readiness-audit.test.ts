...
describe("staging baseline readiness package",()=>{
 ...
 it("never calls pg_get_functiondef for aggregate objects",()=>{expect(production).toContain("p.prokind in ('f','p','w')");expect(production).toContain("'PRODUCTION_AGGREGATES'");expect(production).toContain("p.prokind='a'");const f=production.split("select 'PRODUCTION_FUNCTIONS'")[1].split("select 'PRODUCTION_AGGREGATES'")[0];expect(f).toContain("pg_get_functiondef(p.oid)");const a=production.split("select 'PRODUCTION_AGGREGATES'")[1].split("select 'PRODUCTION_TRIGGERS'")[0];expect(a).not.toContain("pg_get_functiondef")}); it("contains no secret or connection pattern",()=>{const all=[staging,production,plan,incident].join("\n");expect(all).not.toMatch(/postgres(?:ql)?:\/\/|DATABASE_URL|SERVICE_ROLE_KEY|ANON_KEY|eyJ[A-Za-z0-9_-]{20,}/)});
});
