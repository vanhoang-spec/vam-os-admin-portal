...
function fakeClient(seed: { people?: any[]; profiles?: any[] } = {}) {
...
describe("S12-M1 legacy CSV preview", () => {
  ...
  it("uses an encrypted, actor-bound, one-use preview store with no web grants", () => {
    ...
    expect(store).toContain('client.rpc("vam072_consume_legacy_mentor_preview"');
    expect(migration).toContain("delete from public.legacy_mentor_import_previews");
    expect(migration).toContain("enable row level security");
    expect(migration).toContain("revoke all on public.legacy_mentor_import_previews from public, anon, authenticated");
