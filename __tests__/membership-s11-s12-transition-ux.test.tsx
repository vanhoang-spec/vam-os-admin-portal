...
const FIXTURES = [
  {
    name: "Staging Shape",
    UEHM_PROGRAM_ID: STAGING_UEHM_PROGRAM,
    LEGACY_VAM_PROGRAM_ID: STAGING_VAM_PROGRAM,
    UEHM_S11_SEASON_ID: STAGING_S11,
    UEHM_S12_SEASON_ID: STAGING_S12,
  },
  {
    name: "Production Shape",
    UEHM_PROGRAM_ID: PROD_UEHM_PROGRAM,
    LEGACY_VAM_PROGRAM_ID: PROD_UEHM_PROGRAM,
    UEHM_S11_SEASON_ID: PROD_S11,
    UEHM_S12_SEASON_ID: PROD_S12,
  },
];

describe.each(FIXTURES)(
  "Season 11 -> Season 12 Transition UX (%s)",
  ({
    name,
    UEHM_PROGRAM_ID,
    LEGACY_VAM_PROGRAM_ID,
    UEHM_S11_SEASON_ID,
    UEHM_S12_SEASON_ID,
  }) => {
    const PROGRAMS = [
    ...
    const SEASONS = [
      {
        id: UEHM_S11_SEASON_ID,
        label: "Mùa 11",
        code: "UEHM-S11",
        programId: LEGACY_VAM_PROGRAM_ID,
      },
      {
        id: UEHM_S12_SEASON_ID,
        label: "Mùa 12",
        code: "UEHM-S12",
        programId: UEHM_PROGRAM_ID,
      },
    ];
    ...
    it("displays 'Tiếp tục sang Mùa 12' when person has S11 role but no S12 role", () => {
      ...
      expect(
        screen.getByText("mentor · Chưa chuyển sang Mùa 12"),
      ).not.toBeNull();
    ...
    it("applies the same UX to Mentee roles", () => {
      ...
      expect(
        screen.getByText("mentee · Chưa chuyển sang Mùa 12"),
      ).not.toBeNull();
      expect(
    ...
    it("submits normalized lowercase role and exact targets in continuation form (Mentor)", () => {
      ...
      const programIdInput = container.querySelector(
    ...
    describe("S12 Continuation Season Identity Remediation", () => {
        ...
        const WEIRD_LINK_SEASONS = [
          {
            ...
            label: "Mùa 11",
            code: "UEHM-S11",
            programId: "SOME_OTHER_PROGRAM_ID",
          },
          {
            id: UEHM_S12_SEASON_ID,
            label: "Mùa 12",
      ...
      it("does not render CTA when UEHM program is missing from catalog", () => {
        const NO_UEHM_PROGRAMS = [
          { id: LEGACY_VAM_PROGRAM_ID, label: "VAM", code: "VAM" },
        ];
        const memberships = [
          {
            id: "M1",
      ...
      it("hidden season_id targets exact UEHM-S12 season ID in continuation form", () => {
        ...
        const seasonIdInput = container.querySelector(
          'input[name="season_id"]',
        );
        expect(seasonIdInput).not.toBeNull();
        expect(seasonIdInput?.getAttribute("value")).toBe(UEHM_S12_SEASON_ID);
      });
    ...
    it("does not suppress UEHM continuation if person has BKM/HUB mentor memberships", () => {
      const memberships = [
        {
          id: "M1",
          role: "mentor",
...
describe("Anti-Regression Guards", () => {
  it("Staging and Production IDs are disjoint", () => {
    const stagingIds = [
      STAGING_UEHM_PROGRAM,
      STAGING_VAM_PROGRAM,
      STAGING_S11,
      STAGING_S12,
    ];
    const prodIds = new Set([PROD_UEHM_PROGRAM, PROD_S11, PROD_S12]);
