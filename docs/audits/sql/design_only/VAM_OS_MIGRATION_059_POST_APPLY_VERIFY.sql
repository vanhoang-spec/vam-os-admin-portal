...
-- its exact definition; and both enums for exact ordered values.
--
-- Security is ONE uniform contract across all five tables — applications,
-- application_answers, application_reviews, application_decisions and
-- review_assignment_batches. Each is checked for: RLS enabled, RLS FORCED,
-- ZERO policies, no privilege of any kind for PUBLIC, anon or authenticated,
-- and exactly SELECT/INSERT/UPDATE/DELETE for service_role with no TRUNCATE,
-- REFERENCES or TRIGGER. The application workflow is server-only, so there is
-- nothing left to express as a policy.
--
-- This verifier therefore no longer expects public.is_admin_role, no longer
-- expects an authenticated SELECT grant, and no longer expects any policy on
-- any workflow table. A policy of any name on any of the five tables is a
-- FAIL, which is strictly stronger than comparing an expected policy
-- expression: there is no expression left that could hide an extra branch.
--
