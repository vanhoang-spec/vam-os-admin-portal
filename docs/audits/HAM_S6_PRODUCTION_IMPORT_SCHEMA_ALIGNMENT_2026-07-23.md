...
| `mentee_profiles.status` | `mentee_profiles` | Lifecycle status field (e.g. `active`) | `person_season_memberships.status` — check-constrained text field with lifecycle values (`active`, `paused`, `withdrawn`, `completed`, etc.); `mentee_status` (a separate column) also exists and is kept | Remove `status` from `mentee_profiles` INSERT; keep `mentee_status`; lifecycle status is recorded in `person_season_memberships` by module 04 | Migration 052 creates `person_season_memberships` with `status check(status in ('invited','active','paused','withdrawn','completed','graduated','opted_out','cancelled'))` |
...
| `public.mentee_profiles` | 55–60 mentee profiles | — | No `status`; `mentee_status = 'active'` is set; lifecycle status in `person_season_memberships` |
...
### `person_season_memberships.status` for `mentee_profiles.status`

The same membership INSERT above writes `status = 'active'` for each HAM-S6 person.
The `mentee_profiles` table retains `mentee_status = 'active'` as a profile-level
operational status (this column exists and is populated).
