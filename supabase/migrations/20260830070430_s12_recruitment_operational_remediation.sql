...
begin
  ...
  select au.id, au.auth_user_id, au.role
    into v_admin_id, v_existing_auth, v_existing_role
  from public.admin_users au
...
begin
  ...
  if p_new_status = 'under_data_check' then
    ...
    return query select
      v_app.status in ('screening_completed','needs_admin_review') and v_profile_submitted >= v_profile_required,
      case when v_profile_submitted < v_profile_required then 'profile_review_minimum_not_met'
           when v_app.status not in ('screening_completed','needs_admin_review') then 'invalid_transition'
           else 'eligible' end;
    ...
    return query select
      v_app.status = 'ready_for_final_decision' and v_interview_submitted >= v_interview_required,
      case when v_interview_submitted < v_interview_required then 'interview_review_minimum_not_met'
           when v_app.status <> 'ready_for_final_decision' then 'invalid_transition'
           else 'eligible' end;
