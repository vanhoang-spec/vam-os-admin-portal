...
async function logOutboundEmail(row: {
  ...
  // ── main-only (claim-before-send). Giữ hunk này khi merge stack. ──────────
  /**
   * Id của dòng đã được đặt chỗ TRƯỚC khi gửi. Khi có, ghi đè dòng đó thay vì
   * chèn dòng mới — nếu chèn thì mỗi lần gửi sẽ có hai dòng, và index unique
   * chống trùng sẽ từ chối dòng thứ hai.
   */
  ...
  // ── main-only (claim-before-send). Giữ hunk này khi merge stack. ──────────
  //
  // Đường đặt-chỗ-trước: người gọi đã chèn một dòng `queued` và giữ được nó,
  // nên ở đây chỉ chốt kết quả lên chính dòng ấy. `.eq("status", "queued")`
  // khiến một tiến trình thứ hai không thể ghi đè kết quả đã chốt.
  if (row.claimedRowId) {
    const { error: claimError } = await client
      ...
      .eq("id", row.claimedRowId)
      .eq("status", "queued");
    if (claimError) {
      console.error("[email] outbound_emails claim update failed", {
...
async function deliver(
  ...
  relation?: EmailRelation | null,
  batchId?: string | null,
  // ── main-only (claim-before-send). Giữ tham số này khi merge stack. ────────
  claimedRowId?: string | null
 ...
 */
/**
 * Gửi thư báo đổi lịch cho một người đang giữ vé.
 *
 * Ghi sổ gắn với dòng đăng ký, đúng như thư xác nhận: người vận hành tra một
 * địa chỉ trong Nhật ký gửi phải thấy đủ mọi thư đã đi tới họ về buổi này,
 * không phải mỗi thư đầu tiên.
 */
export async function sendEventScheduleChange(input: {
  toEmail: string;
  recipientName: string;
  eventName: string;
  whenLabel: string;
  ...
  const built = buildEventScheduleChangeEmail(input);
  return deliver(
...
/**
 * Gửi thư nhắc lịch cho một người đang giữ chỗ.
 *
 * Ghi sổ gắn với dòng đăng ký như thư xác nhận và thư đổi lịch, để tra một địa
 * chỉ trong Nhật ký gửi thấy đủ mọi thư đã đi tới họ về buổi này.
 */
