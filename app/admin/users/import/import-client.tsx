...
function SubmitButton({ children, danger = false }: { children: React.ReactNode; danger?: boolean }) {
  ...
  return <button disabled={pending} className={danger ? "min-h-11 rounded-md bg-vam-green px-4 py-2 font-medium text-white disabled:opacity-50" : "min-h-11 rounded-md border border-vam-line bg-white px-4 py-2 font-medium text-vam-green disabled:opacity-50"}>{pending ? "Đang xử lý…" : children}</button>;
}
