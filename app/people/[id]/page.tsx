    ...
    <>
      ...
      <Card className="mb-4">
        ...
        <MembershipLifecycleControls personId={params.id} enabled={canOperateAnyScope(scopeContext)} canOperateUehmS12={canOperateUehmS12}
          memberships={seasonMembershipRows.map((row) => ({ id: row.id, role: row.role, status: row.status, intakeBatchCode: row.intake_batch_code, programLabel: String(row.program?.name ?? row.program?.code ?? row.program_id), seasonLabel: String(row.season?.name ?? row.season?.code ?? row.season_id), authorizationScopeLevel: resolveCanonicalScope(scopeContext, row.program?.id, row.program?.code, row.season?.id, row.season?.code), programCode: row.program?.code, seasonCode: row.season?.code, seasonId: row.season_id }))}
          programs={programs.data.map((row) => ({ id: row.id, label: String(row.name ?? row.code ?? row.id), code: row.code ?? undefined }))}
          seasons={seasons.data.map((row) => ({ id: row.id, label: String(row.name ?? row.code ?? row.id), code: row.code ?? undefined, programId: (row as any).program_id ?? undefined }))} />
