...
export async function transitionMembershipAction(_previous: MembershipLifecycleActionState, formData: FormData): Promise<MembershipLifecycleActionState> {
  ...
  const { data: membership, error: membershipError } = await client.from("person_season_memberships").select("id,person_id,program_id,season_id,status").eq("id", membershipId).eq("person_id", personId).maybeSingle();
  ...
  return { ok: true, outcome, message: outcome === "noop" ? "Membership đã ở trạng thái yêu cầu; không tạo log trùng." : "Đã cập nhật membership và ghi lịch sử kiểm toán." };
}
